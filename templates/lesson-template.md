...
至少显式记录对 AMC / IMO / CMO / CEMC / UKMT / 国内正式考试等与本讲相关来源的审查结果。
...
**不能因为本讲没采用 AMC / IMO / CMO，就让这些权威题源在 provenance 中静默消失；也不能为了让赛事名出现在文件里而强行使用超纲题。**
...
只有 R0、R0.5 正确，R7 取舍透明，R11 通过，且最终结论达到 `PASS`（或完成 minor fixes 后 PASS），才把课程标记为完成。
